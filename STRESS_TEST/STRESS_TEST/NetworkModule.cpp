...
void SendPacket(int cl, void* packet)
{
	...
	if (g_clients[cl].client_socket != INVALID_SOCKET) {
...
void ProcessPacket(int ci, unsigned char packet[])
{
	...
	case SC_PARTY_REQUEST: {
		// 자동 수락 또는 거절 (50% 확률)
		CS_PARTY_RESPONSE_PACKET rsp;
		rsp.size = sizeof(rsp);
		rsp.type = CS_PARTY_RESPONSE;
		rsp.acceptFlag = (rand() % 2 == 0);
	...
	case SC_PARTY_UPDATE: {
		SC_PARTY_UPDATE_PACKET* update = reinterpret_cast<SC_PARTY_UPDATE_PACKET*>(packet);
		if (update->memberCount > 6) {
			MessageBox(hWnd, L"Party Size Error", L"ERROR", 0);
			while (true);
		}
	...
	case SC_PARTY_DISBAND: {
		break;
	}
	case SC_ATTACK_NOTIFY: break;
	case SC_ADD_ITEM: break;
	case SC_USE_ITEM_OK: break;
	case SC_USE_ITEM_FAIL: break;
...
void Adjust_Number_Of_Client()
{
	...
	ServerAddr.sin_addr.s_addr = inet_addr(gServerIp.c_str());
